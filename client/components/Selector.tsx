...
const Selector = ({
  ...
  const updateTables = (arg: string | undefined): void => {
    ...
    axios({
      method: 'post',
      url: 'https://saamsa.io/kafka/updateTables',
  ...
  const createTable = (): void => {
    ...
    if (!serverList.includes(`${newBootstrap?.value.replace(':', '_')}`)) {
      axios({
        url: 'https://saamsa.io/kafka/createTable',
        method: 'post',
  ...
  const fetchTables = (): void => {
    axios({
      method: 'post',
      url: 'https://saamsa.io/kafka/fetchTables',
      data: { currentUser },
  ...
  const fetchTopics = (arg: string) => {
    axios({
      url: 'https://saamsa.io/kafka/fetchTopics',
      method: 'post',
      data: { bootstrap: arg, currentUser },
    }).then((response) => {
  ...
  const fetchConsumers = (arg: string) => {
    axios({
      url: 'https://saamsa.io/kafka/fetchConsumers',
      method: 'post',
      data: { bootstrap: arg, currentUser },
    }).then((response) => {
  ...
  const changeTopics = (): void => {
    ...
    if (bootstrap.length && newTopic?.value.length) {
      ...
      axios({
        method: 'POST',
        url: 'https://saamsa.io/kafka/refresh',
